...
/*
 extension SchoolFeedDetailViewController: KeyboardNotificationProtocol {

     func updateToolbarBottomConstant(constant: CGFloat) {
         commentWriteView.snp.updateConstraints {
             $0.bottom.equalTo(self.view.safeAreaLayoutGuide.snp.bottomMargin).offset(-constant)
         }
     }
     ...
     @objc func keyboardWillShow(notification: Notification) {
         if let userInfo = (notification as NSNotification).userInfo {
           if let keyboardHeight = (userInfo[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.size.height {
             updateToolbarBottomConstant(constant: keyboardHeight - self.view.safeAreaInsets.bottom)
     ...
     @objc func keyboardWillHide(notification: Notification) {
         if let userInfo = (notification as NSNotification).userInfo {
           if let _ = (userInfo[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.size.height {
             updateToolbarBottomConstant(constant: 0)
