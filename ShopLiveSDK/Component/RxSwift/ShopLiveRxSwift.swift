...
@objc public final class ShopLiveRxSwift: NSObject {
    ...
    var _style: BehaviorRelay<ShopLive.PresentationStyle> = .init(value: .unknown)
    var _authToken: BehaviorRelay<String?> = .init(value: nil)
    var _user: BehaviorRelay<ShopLiveUser?> = .init(value: nil)

    var liveStreamViewController: LiveStreamViewControllerRxSwift?
    ...
    private func setupRx() {
        ...
        NotificationCenter.default.rx.notification(UIApplication.didBecomeActiveNotification).observe(on: MainScheduler.instance)
                    .asDriver(onErrorRecover: { _ in .never()})
                    .drive(onNext: { [weak self] _ in
                        guard self?._style.value != .unknown else { return }
    ...
    func showShopLiveView(with overlayUrl: URL, _ parent: UIViewController?) {
        guard _style.value == .unknown else {
        ...
        liveStreamViewController = LiveStreamViewControllerRxSwift()
        liveStreamViewController?.delegate = self
        liveStreamViewController?.webViewConfiguration = _webViewConfiguration
        liveStreamViewController?.viewModel.overayUrl = overlayUrl
        liveStreamViewController?.viewModel.authToken = _authToken.value
        liveStreamViewController?.viewModel.user = _user.value
...
//            guard let parent = parent, let liveVC = liveStreamViewController else { return }
//            parent.present(liveVC, animated: true, completion: nil)
//        }
                _style.accept(.fullScreen)
    }
    ...
    private func startCustomPictureInPicture(with position: ShopLive.PipPosition = .default, scale: CGFloat = 2/5) {
        ...
        if liveStreamViewController?.isReplayMode ?? false {
            ...
            UIView.animate(withDuration: 0.3, delay: 0, options: []) {
                self.liveStreamViewController?.isHiddenOverlay.accept(true)
...
//                snapshop.center = shopLiveWindow.center
        ...
        if liveStreamViewController?.isReplayMode ?? false {
            ...
            UIView.animate(withDuration: 0.3, delay: 0, options: []) {
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .milliseconds(100), execute: {
                    self.liveStreamViewController?.isHiddenOverlay.accept(false)
                    self.liveStreamViewController?.view.isHidden = false
                    snapshop.removeFromSuperview()
                })
            }
    ...
    @objc private func liveWindowPanGestureHandler(_ recognizer: UIPanGestureRecognizer) {
        guard _style.value == .pip else { return }
    ...
    @objc private func swipeDownGestureHandler(_ recognizer: UISwipeGestureRecognizer) {
        guard _style.value == .fullScreen else { return }
...
extension ShopLiveRxSwift: ShopLiveComponent {
    ...
    @objc func play(with campaignKey: String?, _ parent: UIViewController? = nil) {
        ...
        fetchOverlayUrl(with: campaignKey) { (overlayUrl) in
            ...
            showShopLiveView(with: url, parent)
        }
    }
...
extension ShopLiveRxSwift: AVPictureInPictureControllerDelegate {
...
extension ShopLiveRxSwift: LiveStreamViewControllerDelegate {
